...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < catalogoRows.length; i += BATCH) {
      await (serviceClient as any).from('catalogo').insert(catalogoRows.slice(i, i + BATCH))
    }
    ...
    for (let i = 0; i < vendasRows.length; i += BATCH) {
      await (serviceClient as any).from('vendas').insert(vendasRows.slice(i, i + BATCH))
    }
    ...
    for (let i = 0; i < estoquesRows.length; i += BATCH) {
      await (serviceClient as any).from('estoques').insert(estoquesRows.slice(i, i + BATCH))
    }
    ...
    for (let i = 0; i < sugestoesRows.length; i += BATCH) {
      await (serviceClient as any).from('sugestoes').insert(sugestoesRows.slice(i, i + BATCH))
    }
    ...
    if (comprasRows.length > 0) {
      for (let i = 0; i < comprasRows.length; i += BATCH) {
        await (serviceClient as any).from('compras_cd').insert(comprasRows.slice(i, i + BATCH))
      }
    }
    ...
    if (semGiroRows.length > 0) {
      await (serviceClient as any).from('sem_giro_cd').insert(semGiroRows)
    }
